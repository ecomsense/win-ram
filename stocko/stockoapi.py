...
class AlphaTrade(Connect):
    ...
    def __init__(
        ...
        self.__exchange_price_multipliers = {
            1: 100,
            2: 100,
            3: 10000000,
            4: 100,
    ...
    def __set_access_token(self):
        try:
            ...
            print(f"Couldn't get profile info {e} ")
            print(f"Creating fresh token.. {e}")
    ...
    def __format_candles(self, data, divider=1):
        ...
        df[["open", "high", "low", "close"]] = (
            df[["open", "high", "low", "close"]].astype(float).div(divider)
        )
    ...
    def __convert_oi(self, dictionary):
        if "instrument" in dictionary:
            dictionary["current_oi"] = int(
                dictionary["current_oi"] / dictionary["instrument"].lot_size
            )
            dictionary["initial_oi"] = int(
                dictionary["initial_oi"] / dictionary["instrument"].lot_size
            )
        return dictionary
    ...
    def __convert_exchanges(self, dictionary):
        if "exchange" in dictionary:
            ...
            dictionary["exchange"] = list(d.keys())[
                list(d.values()).index(dictionary["exchange"])
            ]
        return dictionary
    ...
    def __send_heartbeat(self):
        ...
        while True:
            sleep(5)
            self.__ws_send(
                json.dumps(heart_beat), opcode=websocket._abnf.ABNF.OPCODE_PING
            )
    ...
    def start_websocket(
        ...
        url = self.__service_config["socket_endpoint"].format(
            client_id=self.__login_id, access_token=self.__access_token
        )
    ...
    def get_profile(self):
        ...
        return self.__api_call_helper(
            "profile", Requests.GET, {"client_id": self.__login_id}, None
        )

    def get_balance(self):
        """Get balance/margins"""
        return self.__api_call_helper(
            "cashPositions", Requests.GET, {"client_id": self.__login_id}, None
        )
    ...
    def get_balanceV2(self):
        """Get balance/margins"""
        return self.__api_call_helper(
            "cashPositionsV2", Requests.GET, {"client_id": self.__login_id}, None
        )
    ...
    def get_dematholdings(self):
        """Get Demat holdings"""
        return self.__api_call_helper(
            "holdings", Requests.GET, {"client_id": self.__login_id}, None
        )
    ...
    def fetch_live_positions(self):
        ...
        return self.__api_call_helper(
            "positionBook",
            Requests.GET,
            {"client_id": self.__login_id, "type": "live"},
            None,
        )
    ...
    def fetch_netwise_positions(self):
        ...
        return self.__api_call_helper(
            "positionBook",
            Requests.GET,
            {"client_id": self.__login_id, "type": "historical"},
            None,
        )
    ...
    def get_orderbook(self, pending=True):
        ...
        if pending:
            return self.__api_call_helper(
                "order_book",
                Requests.GET,
                {"client_id": self.__login_id, "type": "pending"},
                None,
            )
        else:
            return self.__api_call_helper(
                "order_book",
                Requests.GET,
                {"client_id": self.__login_id, "type": "completed"},
                None,
            )
    ...
    def place_basket_order(self, orders):  ############### work pending
        ...
        for i in orders:
            ...
            if i["product_type"] == ProductType.Intraday:
                ...
                i["product_type"] = (
                    "NRML" if (i["instrument"].exchange == "NFO") else "CNC"
                )
        ...
        if is_co == False:
            if leg_order_id is None:
                ...
                ret = self.__api_call_helper(
                    ...
                    {
                        "oms_order_id": order_id,
                        "leg_order_indicator": leg_order_id,
                        "execution_type": "REGULAR",
                        "client_id": self.__login_id,
                    },
                    None,
    ...
    def cancel_all_orders(self):  ################## PENDING work
        ...
        for c_order in orders["pending_orders"]:
            if c_order["product"] == "BO" and c_order["leg_order_indicator"]:
                r = self.cancel_order(
                    c_order["leg_order_indicator"],
                    leg_order_id=c_order["leg_order_indicator"],
                )
            elif c_order["product"] == "CO":
                r = self.cancel_order(
                    c_order["oms_order_id"],
                    leg_order_id=c_order["leg_order_indicator"],
                    is_co=True,
                )
            else:
    ...
    def subscribe_market_status_messages(self):
        ...
        return self.__ws_send(
            json.dumps({"a": "subscribe", "v": [1, 2, 3, 4, 6], "m": "market_status"})
        )
    ...
    def subscribe_open_interest(self, instrument):
        ...
        return self.__ws_send(
            json.dumps({"a": "subscribe", "v": arr, "m": "open_interest"})
        )
    ...
    def subscribe(self, instrument, live_feed_type):
        ...
        if live_feed_type == LiveFeedType.MARKET_DATA:
            ...
            live_feed_type == LiveFeedType.ORDERUPDATE
        ):  # message_type == "OrderUpdateMessage":
    ...
    def get_instrument_by_symbol(self, exchange, symbol):
        ...
        if exchange not in self.__master_contracts_by_symbol:
            logger.warning(
                f"Cannot find exchange {exchange} in master contract. "
        ...
        for contract in master_contract:
            if isinstance(symbol, list):
                for sym in symbol:
                    if (
                        sym.lower()
                        in master_contract[contract].symbol.split(" ")[0].lower()
                    ):
    ...
    def __get_master_contract(self, exchange):
        ...
        """
        body = {}
        with open(S_DATA + "Stocko_instruments.csv", "r") as file:
                        ...
                        body[sub].append(
                            {
                                ...
                                "lotSize": (
                                    int(row["lot_size"]) if row["lot_size"] else 0
                                ),
    ...
    def get_candles(
        ...
        r = requests.get(
            "https://web.stocko.in/api/v1/charts/tdv",
            params=params_tv,
            headers=self.__headers,
        )
        data = r.json()
    ...
    def buy_bo(
        self, instrument, qty, price, trigger_price, stop_loss_value, square_off_value
    ):  #### pending
